import pytest
from src.models.entities.documento import Documento
...
def test_documento_init(setup_db):
    """
    Verifica que la entidad Documento se inicializa correctamente.
    """
    doc = Documento(
        nombre="test.pdf", extension="pdf", hash="hash_init", tamano=100, ruta_db=":memory:"
    )
...
def test_insertar_documento(setup_db):
    """
    Verifica que el método insertar() guarda el documento en la BD y asigna el ID.
    """
    doc = Documento(
        nombre="manual.pdf", extension="pdf", hash="hash_insertar", tamano=1024, ruta_db=":memory:"
    )
...
def test_existe_documento(setup_db):
    """
    Verifica que el método existe() detecta correctamente un documento por su hash.
    """
    # Documento a insertar
    doc1 = Documento("doc1.txt", "txt", "hash_existente", 500, ruta_db=":memory:")
    doc1.insertar()

    # Documento con el mismo hash para comprobar
    doc2 = Documento("doc2.txt", "txt", "hash_existente", 600, ruta_db=":memory:")
    assert doc2.existe() is True

    # Documento con hash diferente
    doc3 = Documento("doc3.txt", "txt", "hash_nuevo", 700, ruta_db=":memory:")
    assert doc3.existe() is False


def test_eliminar_documento(setup_db):
    """
    Verifica que el método eliminar() borra el documento de la BD.
    """
    doc = Documento("a_borrar.doc", "doc", "hash_borrar", 1234, ruta_db=":memory:")
...
def test_actualizar_documento(setup_db):
    """
    Verifica que el método actualizar() modifica el documento en la BD.
    """
    doc = Documento("original.pdf", "pdf", "hash_original", 2000, ruta_db=":memory:")
    doc.id = doc.insertar()
...
def test_instanciar_documento(setup_db):
    """
    ...
    doc = Documento(
        nombre="", extension="", hash="", tamano=0, id=id_prueba, ruta_db=":memory:"  # Datos vacíos
    )
...
def test_instanciar_documento_no_existente(setup_db):
    """
    Verifica que instanciar() devuelve False si el ID no existe.
    """
    doc = Documento("", "", "", 0, id=999, ruta_db=":memory:")
    exito = doc.instanciar()
