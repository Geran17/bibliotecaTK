...
class FrameSeleccionarGrupos(Frame):
    ...
    def panel_central(self, frame: Frame):
        ...
        # --- Panel de Grupos ---
        container_grupos = LabelFrame(paned_window, text="Grupos", padding=5)
        paned_window.add(container_grupos)

        ent_grupos = Entry(container_grupos, textvariable=self.var_buscar_grupos)
        ent_grupos.pack(fill=X, padx=2, pady=(0, 5))

        self.frame_grupos = ScrolledFrame(container_grupos, padding=(1, 1))
        self.frame_grupos.pack(fill=BOTH, expand=True, padx=2, pady=2)
