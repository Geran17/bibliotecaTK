...
class FrameSeleccionarColecciones(Frame):
    def __init__(self, master=None, **kwargs):
        super().__init__(master=master, **kwargs)

        # variables
        self.var_buscar_documentos = StringVar()
        self.var_buscar_colecciones = StringVar()
        ...
        self.var_check_colecciones: Dict[str, Any] = {}
        # ✅ Flag para controlar cuando estamos cargando
        self.cargando_asociaciones = False
        # ✅ NUEVO: Almacenar todas las colecciones
        ...
        # ✅ NUEVO: Agregar traces para búsqueda
        self.var_buscar_documentos.trace_add('write', lambda *args: self._filtrar_documentos())
        self.var_buscar_colecciones.trace_add('write', lambda *args: self._filtrar_colecciones())
    ...
    def panel_superior(self, frame: Frame):
        label_titulo = Label(frame, text="Asociar colecciones a los documentos", font=FONT_SECTION)
    ...
    def panel_central(self, frame: Frame):
        ...
        container_colecciones = LabelFrame(paned_window, text="Colecciones", padding=PADDING_PANEL)
        paned_window.add(container_colecciones)

        ent_colecciones = Entry(container_colecciones, textvariable=self.var_buscar_colecciones)
        ent_colecciones.pack(fill=X, padx=PADDING_PANEL, pady=(0, PADDING_OUTER))

        self.frame_colecciones = ScrolledFrame(container_colecciones, padding=(PADDING_COMPACT, PADDING_COMPACT))
        self.frame_colecciones.pack(fill=BOTH, expand=True, padx=PADDING_PANEL, pady=PADDING_PANEL)

    def panel_inferior(self, frame: Frame):
        btn_administrar = Button(
            frame,
            text="Administrar Colecciones",
            command=self.on_administrar_colecciones,
    ...
    def procesar_colecciones_seleccionadas(self, id_documento):
        """Retorna una lista con los nombres de las colecciones seleccionadas"""
        for id_col, var in self.var_check_colecciones.items():
            documento_coleccion = DocumentoColeccion(id_documento=id_documento, id_coleccion=id_col)

            # caso: en que no exista
            if not documento_coleccion.existe():
                """Si no existe la relación y está seleccionada, la asociamos."""
                if var.get():
                    documento_coleccion.asociar()

            # caso: en existe
            if documento_coleccion.existe():
                """Si existe la relación y se quitó la selección, la desasociamos."""
                if not var.get():
                    documento_coleccion.desasociar()

    def cargar_asociaciones(self, id_documento: int):
        """
        Esta funcion se encargara de establecer las colecciones
        ...
        if id_documento != 0:
            ...
            # ✅ Cargar las asociaciones correctamente para TODAS las colecciones
            # No solo las visibles, sino todas las que existen
            for id_col, var in self.var_check_colecciones.items():
                documento_coleccion = DocumentoColeccion(
                    id_documento=id_documento, id_coleccion=id_col
                )
                # Establecer el valor según exista o no la asociación
                var.set(value=documento_coleccion.existe())
    ...
    def _cargar_colecciones(self):
        ...
        for widget in self.frame_colecciones.winfo_children():
            widget.destroy()

        # Limpiar el diccionario
        self.var_check_colecciones.clear()

        # Hacemos una consulta para obtener el total de las colecciones
        consulta = Consulta()
        if isinstance(consulta, Consulta):
            colecciones = consulta.get_colecciones()

            # Guardar referencia a las colecciones
            self.colecciones = colecciones

            # ✅ Aplicar filtro si hay texto en búsqueda
            self._mostrar_colecciones(colecciones)

    def _mostrar_colecciones(self, colecciones: List[Coleccion]):
        """Muestra las colecciones en el grid"""
        # Limpiar widgets existentes
        for widget in self.frame_colecciones.winfo_children():
            widget.destroy()
        ...
        for i, coleccion in enumerate(colecciones):
            ...
            if coleccion.id not in self.var_check_colecciones:
                var = BooleanVar(value=False)
                var.trace_add('write', lambda *args: self.on_coleccion_seleccionado())
                self.var_check_colecciones[coleccion.id] = var
            else:
                var = self.var_check_colecciones[coleccion.id]

            # Cargamos los check
            check = Checkbutton(
                self.frame_colecciones,
                text=coleccion.nombre,
                variable=var,
            )
            check.grid(column=columna, row=fila, sticky=W, padx=5, pady=2)

    def _filtrar_colecciones(self):
        """Filtra las colecciones según el texto de búsqueda"""
        texto_busqueda = self.var_buscar_colecciones.get().lower().strip()

        if not texto_busqueda:
            # Si no hay texto, mostrar todas
            colecciones_filtradas = self.colecciones
        else:
            # Filtrar colecciones que contengan el texto
            colecciones_filtradas = [
                col for col in self.colecciones if texto_busqueda in col.nombre.lower()
            ]

        # ✅ Activar flag para evitar procesamiento durante redibujado
        self.cargando_asociaciones = True
        self._mostrar_colecciones(colecciones_filtradas)
        self.cargando_asociaciones = False
    ...
    def on_coleccion_seleccionado(self):
        """Detecta cambios en las colecciones seleccionadas"""
        ...
        if id_documento != 0:
            self.procesar_colecciones_seleccionadas(id_documento=id_documento)
        else:
            showinfo(
    ...
    def on_administrar_colecciones(self):
        dialog = DialogAdministrarColeccion(master=self.master)
